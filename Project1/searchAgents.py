...
def cornersHeuristic(state, problem):
    ...
    # 启发函数是当前状态距离最终状态的距离
    # 我们选择探索 探索  距离最远的两个未访问节点 的距离的最小值作为启发函数的函数值
    # 其实就是  距离最远的两个未访问角落+当前点到两个点中较近的一个点的距离
    # 如果是只剩一个角落了 那更简单 直接返回当前点到角落的距离就好了

    # 之前的一个想法是   剩余角落之间的距离是恒定的 那么我们可以采用当前节点  距离 剩余角落的点的距离的最小值作为启发函数的函数值
    # 但是很可惜 有一个测试点过不去
    x, y = state[0]
    ...
    #         tmp.append((manhattanDistance, corner))
    #     manhattanDistance, corner = min(tmp)
    #     distance += manhattanDistance
    #     x = corner[0]
    #     y = corner[1]
    #     nonVisitedCorners.remove(corner)
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState: pacman.GameState):
        ...
        fringe.push((startPosition, actions), 0)
        while not fringe.isEmpty():
            state, actions = fringe.pop()
            if state in visited:
                continue
            visited.append(state)
            if food[state[0]][state[1]]:
                return actions
